...
public class FieldMargem extends EditText {
	/**
	 ...
	 * @param defStyle
	 */
	public FieldMargem(Context context, AttributeSet attrs, int defStyle) {
	 ...
	 * @param attrs
	 */
	public FieldMargem(Context context, AttributeSet attrs) {
		super(context, attrs);
	 ...
	 * @param context
	 */
	public FieldMargem(Context context) {
		super(context);
		...
		this.addTextChangedListener(new TextWatcher() {
			public void afterTextChanged(Editable s) {
				...
				if (ls_valor_original.length() < 8) {
					...
					ls_mascara.insert(ls_mascara.length(), "");
					...
					// Roda o novo valor.
					//
					FieldMargem.this.setText(ls_mascara + "%");

					//
					// Faz a seleção.
					//
					FieldMargem.this.setSelection(ls_mascara.length());
				}
			}
	...
	public String getValue() {
		...
		String valor = this.getText().toString().replace(".", "")
				.replace(",", ".").replace("%", "").replace(" ", "");

		// this.getText().replace(0, 3, "").toString().replace(".", "")
		// .replace(",", ".").replace("%", "").replace(" ", "");
	...
	public void setValuePreference(Float valor) {

		FieldMargem.this.setText(String.format("%.2f", valor) + "%");
	}
